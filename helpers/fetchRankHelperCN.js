...
const baseurl = process.env.LEETCODE_HTTP_CONTEST_BASE_URL_CN;

async function postContestData(contestID, data) {
  const bulkOps = data.map(({ username, ranking, score, finishTime, region }) => ({
    updateOne: {
      filter: { contestID, username, region },
      update: {
        $set: { contestID, username, ranking, score, finishTime, region },
      },
      upsert: true,
    },
  }));
...
async function fetchNextPage(url, page, contestID) {
  ...
  for (let i = 0; i < ranks.length; i++) {
    ...
    contest_data.push({
      ...
      ranking: ranks[i].rank,
      score: ranks[i].score,
      finishTime: ranks[i].finish_time,
      region: ranks[i].data_region,
    });
  }
...
async function fetchRankHelperCN(contestID) {
  try {
